...
class CompanyFunding(BaseModel):
    ...
    tavily_reason: str | None = Field(
        default=None, description="Explanation when Tavily verification fails."
    )
    proof_links: list[str] = Field(default_factory=list)
    confidence: str | None = Field(
        default=None, description="Confidence label (VERIFIED/LIKELY/EXCLUDE)."
    )
